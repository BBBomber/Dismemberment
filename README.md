...
If it hit a skeletal mesh, it finds the nearest bone, walks up to the nearest valid cut point, hides that part of the body, builds a matching chunk from the mesh data and spawns it as a separate actor at the correct location, then drops the body into ragdoll. Blood spawns at the cut point. The ragdoll can be cut up until it has no severable bones left. Currently this does not support a cap mesh on the spawned limb chunk. 
