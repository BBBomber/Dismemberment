...
void USkeletalMeshDismembermentComp::ProcessHit_Implementation(const FDismembermentHitData& HitData)
{
    ...
    SpawnBloodEffects(SkelMesh, SeveranceBone);
...
void USkeletalMeshDismembermentComp::SpawnDetachedChunk(USkeletalMeshComponent* SkelMesh, FName SeveranceBone) const
{
    ...
    if (SeveranceBoneIndex == INDEX_NONE)
        return;
...
void USkeletalMeshDismembermentComp::SpawnBloodEffects(USkeletalMeshComponent* SkelMesh, FName SeveranceBone) const
{
    ...
    if (BloodDecalMaterial)
    {
        ...
        const FVector TraceEnd = BoneLocation + FVector(0.f, 0.f, -600.f);
        if (GetWorld()->LineTraceSingleByChannel(GroundHit, BoneLocation, TraceEnd, ECC_WorldStatic, TraceParams))
        {
            FRotator DecalRotation = GroundHit.ImpactNormal.Rotation();
            DecalRotation.Pitch -= 90.f;

            UGameplayStatics::SpawnDecalAtLocation(
                GetWorld(),
                BloodDecalMaterial,
                FVector(60.f, 60.f, 5.f),
